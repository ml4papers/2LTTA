...
parser.add_argument("--json-root", type=str, default='/TTA/READ/_code_clean/json_csv_files/vgg', help="validation data json")
parser.add_argument("--label-csv", type=str, default='/TTA/READ/_code_clean/json_csv_files/class_labels_indices_vgg.csv', help="csv with class labels")
...
parser.add_argument("--pretrain_path", type=str, default='/TTA/READ/_code_clean/pretrained_model/vgg_65.5.pth', help="pretrained model path")
...
parser.add_argument('--tta-method', type=str, default='2LTTA', choices=['2LTTA', 'Tent', 'SAR', 'None'], help='which TTA method to be used')
...
for corruption in corruption_list:
    for severity in range(args.severity_start, args.severity_end+1):
        ...
        for itr in range(1, 6):
            ...
            if args.tta_method == '2LTTA' or args.tta_method == 'None':

                va_model = TLTTA.configure_model(va_model)
                ...
                params, param_names = TLTTA.collect_params(va_model)

                optimizer = torch.optim.Adam([{'params': params, 'lr': 1e-4}],
                                             weight_decay=0., 
                                             betas=(0.9, 0.999))

                read_model = TLTTA.TLTTA(va_model, optimizer, device, args)
                read_model.eval()
                overlap = []
                with torch.no_grad(): 
                    for epoch in range(1):
                        ...
                        for i, (a_input, v_input, labels) in enumerate(data_bar):
                            ...
                            data_bar.set_description(f'Batch#{i}: L0#{loss:.4f} ACC#{batch_acc:.2f}')
